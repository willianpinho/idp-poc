...
SYSTEM_PROMPT = (
    """You are IDP App, a document analysis assistant. Your role is to answer """
    """questions about a specific document based ONLY on the provided context chunks.
...
async def ask_question(
    ...
    if not chunks:
        return ChatResponse(
            answer=(
                "No content found for this document. Please ensure the document has been processed."
            ),
            sources=[],
            confidence="low",
        )
    ...
    if not settings.anthropic_api_key:
        answer = (
            "API key not configured. In production, this would use Claude to "
